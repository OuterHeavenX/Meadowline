...
MESH_OUT = os.path.join(REPO, "src", "rendering", "assets", "tree-lowpoly.mesh.js")
...
def write_mesh_module(tree):
    """Emit the mesh as an ES module of plain arrays.

    The .glb is the interchange artifact and stays authoritative, but the game
    loads this instead. Meadowline has no build step and a synchronous render
    path: vendoring GLTFLoader means 180 KB of unminified loader with Draco,
    KTX2 and skinning support for one static mesh, and fetching a .glb means
    the first frames draw a tree that is not there yet. A module is imported
    like every other part of the renderer and is simply present.

    Flat shading means face normals, so vertices are not shared and the mesh is
    written unindexed. Grouped by material, in the order the materials sit on
    the object, so the runtime can build one geometry with three groups and
    draw every tree in the city as a single instanced mesh.
    """
    mesh = tree.data
    ...
    positions, norms, ranges = [], [], []
    for index, triangles in enumerate(groups):
        start = len(positions) // 3
        for triangle in triangles:
            for loop_index in triangle.loops:
                vertex = mesh.vertices[mesh.loops[loop_index].vertex_index]
                normal = normals[loop_index].vector if hasattr(normals[loop_index], "vector") else normals[loop_index].normal
                # Blender is Z-up, three.js is Y-up. Same swap the glTF
        ...
        ranges.append((start, len(positions) // 3 - start, index))

    def fmt(values):
        return ",".join(f"{v:.4f}".rstrip("0").rstrip(".") or "0" for v in values)

    colours = [m.name for m in mesh.materials]
    hexes = {"MeadowlineTrunk": TRUNK, "MeadowlineCanopy": CANOPY,
             "MeadowlineCanopyHighlight": CANOPY_HI}

    os.makedirs(os.path.dirname(MESH_OUT), exist_ok=True)
    with open(MESH_OUT, "w") as f:
        f.write("/* Generated by assets/source/blender/tree_lowpoly.py - do not edit.\n")
        f.write("   Regenerate with: python3 assets/source/blender/tree_lowpoly.py\n\n")
        f.write("   The same mesh as assets/models/vegetation/tree-lowpoly.glb, in the\n")
        ...
        f.write("   by material so the whole city's trees draw as one instanced mesh. */\n")
        f.write(f"export const TRIANGLES={len(mesh.loop_triangles)};\n")
        f.write("export const COLORS=[" + ",".join(f"'{hexes[c]}'" for c in colours) + "];\n")
        f.write("export const GROUPS=" + repr([list(r) for r in ranges]).replace(" ", "") + ";\n")
...
def main():
    ...
    export(tree)
    verts = write_mesh_module(tree)
    print(f"triangles      {tris} (budget 500)")
    print(f"mesh module    {verts} vertices")
