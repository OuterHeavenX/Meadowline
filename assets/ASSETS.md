...
- `assets/models/civic/statue.glb`, `clockTower.glb`, `lighthouse.glb`, `greatLibrary.glb` and `assets/models/buildings/farm.glb`, `mill.glb` — the four Wonders and the two ends of the food chain, 106 to 872 triangles each, all inside the budgets above. Original work for this repository. Source of record is `assets/source/blender/landmarks.py`, which builds every model from an empty scene with no randomness anywhere in it, so a regenerated asset diffs cleanly or not at all. Run `python3 assets/source/blender/landmarks.py` for all of them or name the ones you want.
...
- Two parts are deliberately left out of the meshes and still drawn by the renderer: the windmill's sails, which turn, and the traffic signals' lamps, which change colour. A fixed mesh can do neither. The models leave a hub and a post for them to attach to.
