...
export function save(){
  ...
  const payload=JSON.stringify({v:3,w:W,h:H,seed:S.seed,coins:Math.floor(S.coins),day:S.day,dayT:S.dayT,b,woods,terrain:packTerrain(),tutorial:S.tutorial,municipal:S.municipal,social:{...packSocial(),...packFamilies(),...packCareers(),...packOrganisations(),...packFame()},quality:S.quality,rendererMode:S.rendererMode,muted:!!S.muted,cityProgress:sanitizeProgression(S.cityProgress,false),mile:mileHit,granted:S.granted||0,wishes:S.wishes.map(w=>({k:w.k,slot:w.slot,t:w.t,g:w.g,r:w.r})),log:S.log.slice(0,40),history:S.history.slice(-40)});
...
export function applySave(d){
  ...
  restoreSocial(d.social); restoreFamilies(d.social); restoreCareers(d.social); sanitiseFamilyCareers(); restoreOrganisations(d.social); restoreFame(d.social);
