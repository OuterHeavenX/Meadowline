...
// The 44x44 world is divided into deterministic, contiguous development sectors.
// Center is 20x20 (400 tiles, ~21% of the full map); the world itself never changes.
...
export const BUILDING_STAGE=Object.freeze({
  road:1,house:1,cafe:1,park:1,tree:1,lamp:1,
...
function cleanUnlocked(list){
  const valid=new Set(LAND_PARCELS.map(p=>p.id));
  const out=[];
  for(const id of Array.isArray(list)?list:[]) if(valid.has(id)&&!out.includes(id)) out.push(id);
  if(!out.includes('center')) out.unshift('center');
  return out;
}

export function createProgression(mode='parcel'){
  return {mode:mode==='legacy-open'?'legacy-open':'parcel',stage:mode==='legacy-open'?4:1,unlockedParcels:mode==='legacy-open'?LAND_PARCELS.map(p=>p.id):['center'],claimedMilestones:[]};
}

export function sanitizeProgression(raw,legacyFallback=false){
  if(!raw||typeof raw!=='object') return createProgression(legacyFallback?'legacy-open':'parcel');
  ...
  return {
    ...
    unlockedParcels:mode==='legacy-open'?LAND_PARCELS.map(p=>p.id):cleanUnlocked(raw.unlockedParcels),
    claimedMilestones:Array.isArray(raw.claimedMilestones)?raw.claimedMilestones.filter(x=>typeof x==='string').slice(0,32):[]
  };
}
...
export function isTileUnlocked(x,y){
  ...
  return !!p&&isParcelUnlocked(p.id);
}
export function isFootprintUnlocked(x,y,w=1,h=1){
  for(let yy=y;yy<y+h;yy++) for(let xx=x;xx<x+w;xx++) if(!isTileUnlocked(xx,yy)) return false;
  return true;
}
...
export function developmentStats(){
  ...
  let occupiedHomes=0,townHomes=0,establishedHomes=0,desirabilityTotal=0,educationTotal=0;
  for(const h of houses){
    if((h.pop||0)>0) occupiedHomes++;
    const tier=Math.max(1,Math.floor(Number(h.state?.housingTier)||1));
    if(tier>=2) townHomes++;
    if(tier>=3) establishedHomes++;
    desirabilityTotal+=Number(h.state?.desirability)||0;
    educationTotal+=Number(h.state?.education)||0;
  }
  let roads=0;
  for(const b of S.grid||[]) if(b?.type==='road') roads++;
  return {
    population:S.pop||0,
    homes:houses.length,
    occupiedHomes,
...
export function stageProgress(targetStage,stats=developmentStats()){
  const def=STAGE_REQUIREMENTS[targetStage];
  if(!def) return {complete:true,required:[],any:[]};
  const required=def.required.map(r=>check(r,stats));
  const any=def.any.map(g=>{
    const items=g.items.map(r=>check(r,stats));
    return {...g,items,met:items.filter(i=>i.met).length>=g.count};
  });
  return {complete:required.every(r=>r.met)&&any.every(g=>g.met),required,any};
}
...
export function evaluateCityGrowth(){
  ...
  let next=before+1;
  while(next<=4&&stageProgress(next).complete){ S.cityProgress.stage=next; next++; }
  if(S.diagnostics) S.diagnostics.progressionRecomputes=(S.diagnostics.progressionRecomputes||0)+1;
  return {stageChanged:S.cityProgress.stage!==before,from:before,stage:S.cityProgress.stage};
}
...
export function parcelStatus(id){
  ...
  return {parcel,state:stageOk&&prereqOk?'available':'locked',stageOk,prereqOk,coinsOk,canUnlock:stageOk&&prereqOk&&coinsOk};
}
...
export function unlockParcel(id){
  ...
  if(!st||st.state==='unlocked') return {ok:false,why:'That land is already open.'};
  if(!st.stageOk) return {ok:false,why:'Reach '+CITY_STAGES[st.parcel.stage-1].name+' first.'};
  if(!st.prereqOk) return {ok:false,why:'Open the neighboring land first.'};
  if(!st.coinsOk) return {ok:false,why:'You need '+st.parcel.cost+' coins to open this land.'};
  S.coins-=st.parcel.cost;
  S.cityProgress.unlockedParcels=cleanUnlocked([...(S.cityProgress.unlockedParcels||[]),id]);
  if(S.diagnostics) S.diagnostics.parcelUnlocks=(S.diagnostics.parcelUnlocks||0)+1;
  return {ok:true,parcel:st.parcel};
}

export function nextStageProgress(){
  const current=S.cityProgress?.stage||1;
  return current>=4?null:{stage:CITY_STAGES[current],progress:stageProgress(current+1)};
}
