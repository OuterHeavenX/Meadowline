...
function addBuilding(b){const def=getBuildingDefinition(b.type),fp=def?.placement?.footprint||[1,1],cx=b.x+(fp[0]-1)/2,cz=b.y+(fp[1]-1)/2,w=fp[0]*.82,d=fp[1]*.82,type=b.type,seed=b.seed||b.x*31+b.y*17;
  ...
  let h=type==='hospital'?2.1:type==='cityHall'?1.25:type==='fireStation'?1.1:type==='policeStation'?1:type==='school'?1.05:type==='station' ? .82 : .78;if(type==='house')h=[.68,1.02,1.36][clamp((b.state?.housingTier||1)-1,0,2)];
