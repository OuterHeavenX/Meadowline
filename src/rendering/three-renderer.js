...
/* ---------- clouds that cast their own shadows ----------
   Drawing a cloud overhead and a dark patch on the ground as two separate
   things means keeping them in step by hand for ever, and getting the
   occlusion wrong the moment one passes behind a roof. These are real bodies
   in the scene at the height the cloud data already carries, and the sun that
   lights the town casts their shadows for free.

   That also gives the behaviour asked for without a special case: a
   transparent material still casts a full shadow in three.js, so zooming in
   fades the cloud out of the way of the town while the shadow crossing the
   ground stays exactly as strong. */
function cloudMat(){
...
function rebuildDynamic(){if(dynamic)scene.remove(dynamic);dynamic=new THREE.Group();scene.add(dynamic);for(const c of S.citizens){const walk=sidewalkOffset(c),x=(c.facilityLocal?.x??lerp(c.x,c.nx,c.p))+walk.x,z=(c.facilityLocal?.y??lerp(c.y,c.ny,c.p))+walk.y,g=new THREE.Group();g.position.set(x,.08,z);dynamic.add(g);cylinder(0,0,0,.045,.22,mat(c.col||c.color||'#d6a86e'),g,7);mesh(geo('head',()=>new THREE.SphereGeometry(.055,7,5)),mat('#d7a678'),0,.28,0,g,false);}for(const v of S.vehicles||[])addVehicle(v);for(const v of S.serviceVehicles||[])addVehicle(v,true);for(const t of S.trains||[]){const x=t.fx||t.x,z=t.fy||t.y;box(x,.12,z,.72,.28,.34,mat('#b34f42'),dynamic,true);}for(const inc of S.incidents||[]){if(inc.resolved)continue;const x=inc.target.x,z=inc.target.y;if(inc.kind==='fire'){for(let i=0;i<3;i++)pyramid(x+(i-1)*.12,.25,z,.12,.38,mat(i%2?'#ffad3e':'#e34b2f',.25),dynamic,7);}else if(inc.kind==='crime'){cylinder(x,.08,z,.06,.3,mat('#34383b'),dynamic,7);}else{mesh(geo('med',()=>new THREE.OctahedronGeometry(.13)),mat('#e9eeee'),x,.55,z,dynamic,false);}}addClouds();addInteractionOverlay();}
